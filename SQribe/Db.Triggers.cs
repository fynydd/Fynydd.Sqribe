...
namespace SQribe
{
	...
	public class Triggers : ITriggers
	{
        ...
        public Triggers(ISettings singletonSettings, IOutput singletonOutput, IHelpers singletonHelpers)
        {
		...
		/// <summary>
		/// Generate script to create triggers.
		/// </summary>
        ...
        public void DropAll(long token)
        {
            helpers.DropObject(
                token, 
                ",tr,", 
                settings.ScriptPath + settings.TriggersFilename, 
                "trigger", 
                "drops" + Path.DirectorySeparatorChar.ToString() + "drop-triggers.sql"
                );
        }
